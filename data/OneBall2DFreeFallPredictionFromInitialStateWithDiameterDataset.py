...
class OneBall2DFreeFallPredictionFromInitialStateWithDiameterDataset(ClassicalMechanicsDataset):
    def __getitem__(self, idx):
        data = np.load(self.data_files[idx])
        return torch.FloatTensor(data[0]).unsqueeze(dim=0), torch.FloatTensor(data[1:, 0:2]).reshape(1,-1)
    ...
    def generate_data(self, max_actions=30000, **kwargs):
        ...
        def get_red_ball_data(simulation):
            ...
            for frame_id in range(min(25, len(features))):
                data.append([features[frame_id][-1][0], features[frame_id][-1][1], features[frame_id][-1][3]])
            return data
