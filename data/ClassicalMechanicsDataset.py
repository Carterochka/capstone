...
class ClassicalMechanicsDataset(Dataset, metaclass=abc.ABCMeta):
    ...
    @classmethod
    def train_test_split(dataset_cls, path, test_frac=0, random_state=None, max_actions=30000, **kwargs):        
        data_glob = glob(path + '/*')
        
        if not data_glob:
            dataset_cls(path).generate_data(max_actions=max_actions, **kwargs)
            data_glob = glob(path + '/*')
    ...
    @abc.abstractmethod
    def generate_data(self, max_actions=30000, **kwargs): pass